 ...
 *
 * ⚠ 학생이 예시를 고르면 **클래스 이름과 속성까지만** 채워진다 (`designFromIdea()`).
 *   `methods` 는 **일부러 쓰지 않는다.** (2026-07-30 지시)
 *   메소드를 무엇으로 만들지 정하는 것이 학생이 할 설계이고 평가요소 5의 핵심이라서다.
 *   자료는 남겨 두었으니 나중에 「이런 동작이 있을 수 있어요」 같은 힌트로 쓸 수는 있다.
 *   다시 자동으로 채우도록 되돌리지 말 것.
 * ------------------------------------------------------------------------- */
